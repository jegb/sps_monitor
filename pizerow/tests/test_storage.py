...
from pizerow.app.storage import HISTORY_HEADER, StorageManager, format_history_row, history_path_for_timestamp
...
class StorageTests(unittest.TestCase):
    ...
    def test_append_history_writes_single_header(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            store = StorageManager(root=tmpdir, history_enabled=True)
            store.append_history(_record("2026-04-17T00:00:00Z"))
            store.append_history(_record("2026-04-17T00:01:00Z", pm_1_0=1.5))

            path = history_path_for_timestamp(tmpdir, "2026-04-17T00:00:00Z")
